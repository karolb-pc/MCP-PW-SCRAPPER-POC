...
- Keep the current class-based Extract / Transform / Load architecture.
...
- Update the model and validator only if the output contract requires it.
- Keep the local JSON loader behavior intact unless the requested extension requires loader metadata changes.
- If target kind is discovery_run, extend the generated scraper in place under its generated/ directory.
- If target kind is static_etl, extend the existing ETL files listed above.
