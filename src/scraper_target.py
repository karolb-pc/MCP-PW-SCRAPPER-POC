...
def manifest_target(source_path: Path, output_path: Path | None = None) -> ScraperTarget:
    ...
    # Output paths are standardized under the repo-root output/ tree, so resolve a
    # relative manifest output_path against the current working directory (repo root),
    # not the scraper folder. `files`/`allowed_paths` remain relative to the folder.
    ...
    if not configured_output_path.is_absolute():
        configured_output_path = Path.cwd() / configured_output_path
